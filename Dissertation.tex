...
More care is needed for the discrete case. Let u



In the general case for computing $S(s+C,\ell)$, the cone of feasible directions at a vertex $C_s(\PP)$ many not be simplicial. Moreover, the number of lattice points in $\Pi_C \cap \Z^d$ may be exponential in the encoding of $C$. A standard solution to these problems is to compute a triangulation for each cone $C_s$, and then decompose each simplicial cone into unimodular cones via Algorithm \ref{alg:barvinok-dual}.
